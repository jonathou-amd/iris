...
Each persistent step reduces one assigned spatial tile across all ranks:
    for each source rank: TDM load tile -> LDS
    accumulate in fp32 registers (smem.load + add)
    TDM store reduced tile -> local output only

Opposite of all-to-all TDM (1 dest/store per step): here W loads + 1 local store
per tile, with reduction in between.
"""
...
def launch(
    ...
    iris_launch(
        persistent_reduce_scatter_tdm_gfx1250,
        (config.comm_sms,),
        input_tensor,
        output_tensor,
        elem_deltas,
